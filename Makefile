...
install :; forge install Cyfrin/foundry-devops@0.0.11 --no-commit && forge install foundry-rs/forge-std@v1.5.3 --no-commit && forge install openzeppelin/openzeppelin-contracts --no-commit
...
deploy-sepolia:
	@forge script script/DeployOurToken.s.sol --rpc-url $(SEPOLIA_RPC_URL) --broadcast --account sepolia1 --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
